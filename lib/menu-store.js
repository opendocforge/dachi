...
// Ordre d'affichage des actions dans le menu (tableau d'identifiants).
// Clé volontairement hors préfixe "menu_" : ce n'est pas une action.
export const ORDER_KEY = "menuOrder";
...
export const isMenuKey = (key) => key.startsWith(MENU_KEY_PREFIX) || LEGACY_KEYS.includes(key) || key === ORDER_KEY;
...
export async function loadMenuConfig() {
  ...
  const order = Array.isArray(all[ORDER_KEY]) ? all[ORDER_KEY].filter(x => typeof x === "string") : [];
  return { overrides, custom, order };
}

/** Enregistre l'ordre des actions (tableau d'identifiants) ; [] = ordre par défaut. */
export async function saveMenuOrder(ids) {
  const clean = Array.from(new Set((ids || []).filter(x => typeof x === "string")));
  if (!clean.length) await chrome.storage.sync.remove(ORDER_KEY);
  else await chrome.storage.sync.set({ [ORDER_KEY]: clean });
}

/**
 * Trie les actions selon `order` ; les identifiants absents de `order`
 * (nouvelles actions, ordre jamais défini) conservent leur ordre naturel
 * (défauts puis perso) et sont placés à la fin.
 */
export function applyMenuOrder(items, order) {
  if (!Array.isArray(order) || !order.length) return items;
  const rank = new Map(order.map((id, i) => [id, i]));
  return items
    .map((item, i) => ({ item, key: rank.has(item.id) ? rank.get(item.id) : order.length + i }))
...
export function resolveMenuItems(defaults, config) {
  ...
  return applyMenuOrder(items, config.order);
}
