...
export function deltaContentOf(payload) {
  ...
  if (!choice) return { text: "", reasoning: "", finishReason: null };
  const delta = choice.delta || choice.message || {};
  const text = typeof delta.content === "string" ? delta.content : "";
  const r = delta.reasoning_content ?? delta.reasoning;
  const reasoning = typeof r === "string" ? r : "";
  return { text, reasoning, finishReason: choice.finish_reason || null };
}

export const REASONING_LEVELS = ["none", "low", "medium", "high", "max"];

/**
 * Effort de raisonnement à envoyer, parmi ceux que le modèle accepte.
 * requested : "auto" | "none" | "low" | "medium" | "high".
 * "auto" → "none" si possible, sinon le plus faible accepté.
 * Un effort demandé mais non accepté est rapproché du niveau accepté le plus
 * proche vers le bas (la rédaction n'a pas besoin de raisonnement).
 */
export function pickReasoningEffort(requested, supported) {
  if (!Array.isArray(supported) || !supported.length) return null;
  const order = REASONING_LEVELS.filter(l => supported.includes(l));
  if (!order.length) return null;
  const want = requested === "auto" ? "none" : requested;
  if (order.includes(want)) return want;
  const wi = REASONING_LEVELS.indexOf(want);
  const below = order.filter(l => REASONING_LEVELS.indexOf(l) < wi);
  return below.length ? below[below.length - 1] : order[0];
}
