...
Dans la modale de résultat : réponse affichée **en streaming**, boutons **Régénérer**, **Affiner** (consigne supplémentaire appliquée à la réponse précédente), **Copier** et **Remplacer la sélection**. Le dernier résultat peut être rouvert depuis le menu Dachi. **Raccourcis clavier** (modifiables dans `chrome://extensions/shortcuts`) : <kbd>Alt</kbd>+<kbd>C</kbd> Corriger & Reformuler, <kbd>Alt</kbd>+<kbd>X</kbd> Corriger seulement, <kbd>Alt</kbd>+<kbd>R</kbd> Résumer — chaque touche est associée à l'action de votre choix dans les options — et <kbd>Alt</kbd>+<kbd>V</kbd> **remplace la sélection par le résultat**, y compris pendant la génération (le texte est inséré dès qu'il est prêt). Enchaînement type : sélectionner → <kbd>Alt</kbd>+<kbd>C</kbd> → <kbd>Alt</kbd>+<kbd>V</kbd>. Dans la modale, <kbd>Ctrl</kbd>+<kbd>Entrée</kbd> insère aussi.
